--[[
	...
	Version: 1.13.6670 (SwimmingSeadragon)
	Revision: $Id: Configator.lua 6670 2021-01-30 13:42:33Z none $
...
local LIBRARY_VERSION_MINOR = 34
local lib = LibStub:NewLibrary(LIBRARY_VERSION_MAJOR, LIBRARY_VERSION_MINOR)
if not lib then return end

LibStub("LibRevision"):Set("$URL: Auc-Advanced/Libs/Configator/Configator.lua $","$Rev: 6670 $","5.1.DEV.", 'auctioneer', 'libs')
...
function lib:Create(setter, getter, dialogWidth, dialogHeight, gapWidth, gapHeight, topOffset, leftOffset)
	...
	local gui = CreateFrame("Frame", name, UIParent, BackdropTemplateMixin and "BackdropTemplate")
	...
	gui.Proxy = CreateFrame("Frame", name.."Proxy", gui, BackdropTemplateMixin and "BackdropTemplate")
	gui.Proxy.gui = gui
...
if not lib.help then
	...
	lib.help.content = CreateFrame("Frame", "ConfigatorHelpContent", lib.help, BackdropTemplateMixin and "BackdropTemplate")
...
function kit:ZeroFrame()
	...
	content = CreateFrame("Frame", myName.."Content", frame, BackdropTemplateMixin and "BackdropTemplate")
...
function kit:AddControl(id, cType, column, ...)
	...
	elseif (cType == "Checkbox") then
		...
		local indent = 10 * (level or 1)
		-- CheckButton
		el = CreateFrame("CheckButton", nil, content, "OptionsBaseCheckButtonTemplate")
		kpos = kpos+1 kids[kpos] = el
	...
	elseif (cType == "MoneyFrame" or cType == "PinnedMoney" or cType == "MoneyFramePinned") then
		...
		el = CreateFrame("Frame", frameName, content, BackdropTemplateMixin and "BackdropTemplate" and "MoneyInputFrameTemplate")
